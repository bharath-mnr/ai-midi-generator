...
class TextToMidiConverter {
  ...
  // NEW: Expand compression notation
  static expandCompression(symbols) {
    const expanded = [];
    
    for (const symbol of symbols) {
      // Match pattern: symbol(count)
      ...
      if (compressionMatch) {
        ...
        if (count < 1) {
          throw new Error(`Invalid compression count: ${count}. Must be positive integer.`);
        }
        
        // Expand the symbol count times
  ...
  static parseNoteSymbol(symbol, defaultVelocity = 100) {
    ...
    if (sustainMatch) {
      ...
      if (sustainMatch[1]) {
        sustainCutoff = parseInt(sustainMatch[1]);
        sustainCutoff = Math.max(0, Math.min(100, sustainCutoff));
      }
      return { 
        isNoteOn: false, 
        velocity, 
    ...
    if (positionedRestMatch) {
      ...
      restDuration = Math.max(0, Math.min(100, restDuration));
    ...
    if (symbol.startsWith('X')) {
      ...
      if (rightOffsetMatch) {
        timingOffset = parseInt(rightOffsetMatch[1]);
        timingOffset = Math.max(0, Math.min(100, timingOffset));
        symbol = symbol.replace(/XR\d+/, '');
      }
      ...
      if (durationMatch) {
        durationPercent = parseInt(durationMatch[1]);
        durationPercent = Math.max(0, Math.min(100, durationPercent));
        symbol = symbol.replace(/E?\d+/, '');
      }
    }
      ...
      for (const segment of segments) {
        ...
        if (segmentStr.startsWith('XO')) {
          ...
          if (posMatch) {
            ...
            restOffset = Math.max(0, Math.min(100, restOffset));
            noteDuration = Math.max(0, Math.min(100, noteDuration));
  ...
  static parseTextToMidi(text) {
    ...
    lines.forEach(line => {
      ...
      if (line.startsWith('Tempo:')) {
        ...
        // EXPANSION: Expand compressed notation before storing
